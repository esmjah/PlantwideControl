...
if MPC:
    ...
    finalStateCost = 0.0
    solver.defineOCP(ocp, DT=DTMPC, controlCost=controlCost, xOpt=xOpt, uOpt=uOpt, finalStateCost=finalStateCost,
...
if MPC:
    uk, objValue, stats = solver.solveProblem(x0=x_hat, u0=u0)
...
for k in range(k0, NIT+1):
    ...
    if (k > 3600 * 1 / DT) and (k <= 3600 * 11 / DT):
        KF.R = R0 - 0.027 * (k - 3600 / DT) * np.diag(np.ones(len(measurementTagsModelica)))
    ...
    if (np.mod((k - k0MPC) * DT, DTMPC) == 0) and (k >= k0MPC):
        ...
        if exit_msg == 'Infeasible_Problem_Detected' or exit_msg == 'Invalid_Number_Detected':
            ...
            A_inv = np.linalg.inv(-A)
            Ju = np.array(C * (A_inv * B) + D)[0]
    uk_sim = np.concatenate((np.array([160, 170, 0, 0]), np.transpose(np.array(uk))[0]), axis=0)
    ...
    if doPrint and k % 36 == 0:  # and np.mod((k + 1 - k0MPC) * DT, DTMPC / 10.0) == 0:
        ...
        print 'Monitor Process', (yMonitor)  # /monitorScale
