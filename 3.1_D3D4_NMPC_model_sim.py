...
measurementsMonitorModelica = ['net.w1.P_bh','net.w2.P_bh','net.p.w_L_out','net.p.w_G_out','net.PCA1.u','net.PCA2.u','net.PCINL.u','net.p.fin.p','net.w1.GOR_m_t','net.w2.GOR_m_t','net.w1.alpha_G_m_in','net.w2.alpha_G_m_in']
measurementsMonitorSim = ['net.w1sim.P_bh','net.w2sim.P_bh','net.p.w_L_out','net.p.w_G_out','net.PCA1.u','net.PCA2.u','net.PCINL.u','net.p.fin.p','net.w1sim.GOR_m_t','net.w2sim.GOR_m_t','net.w1sim.alpha_G_m_in','net.w2sim.alpha_G_m_in']
...
for k in range(k0, NIT+1):
    ...
    if (np.mod((k - k0MPC) * DT, DTMPC) == 0) and (k >= k0MPC):
        ...
        if exit_msg == 'Infeasible_Problem_Detected' or exit_msg == 'Invalid_Number_Detected':
            ...
            A_inv = np.linalg.inv(-A)
            Ju = np.array(C * (A_inv * B) + D)[0]
    uk_sim = np.concatenate((np.array([160, 170, alpha_Gm1, alpha_Gm2]), np.transpose(np.array(uk))[0]), axis=0)
