...
load_libSplinter()
load 'OptimalSurface_Pres155170.mat';
...
J_Pres155170 = BSplineBuilder(xs, Js, 3).build();
J_Pres155170.save('OptimalSurface_Pres155170.bspline');
GORm1_Pres155170 = BSplineBuilder(xs, GORm1s, 3).build();
GORm2_Pres155170 = BSplineBuilder(xs, GORm2s, 3).build();
Pwh1_Pres155170 = BSplineBuilder(xs, 1e-5*Pwh1s, 3).build();
Pwh2_Pres155170 = BSplineBuilder(xs, 1e-5*Pwh2s, 3).build();
Pinl_Pres155170 = BSplineBuilder(xs, Pinls, 3).build();
Zwh1_Pres155170 = BSplineBuilder(xs, Zwh1s, 3).build();
Zwh2_Pres155170 = BSplineBuilder(xs, Zwh2s, 3).build();
Ztop_Pres155170 = BSplineBuilder(xs, Ztops, 3).build();
WLwh1_Pres155170 = BSplineBuilder(xs, WLwh1s, 3).build();
WLwh2_Pres155170 = BSplineBuilder(xs, WLwh2s, 3).build();
WGwh1_Pres155170 = BSplineBuilder(xs, WGwh1s, 3).build();
WGwh2_Pres155170 = BSplineBuilder(xs, WGwh2s, 3).build();
% save('IPR_165_2D_Splines_mass.mat','WL_w165','QL_w165','WG_w165','QG_w165','PWH_w165','TWH_w165')

J_eval = J_Pres155170.eval([WG1_min WG2_min])

%%
Error = zeros(NW1,NW2);
for j = 1:NW2
    for i = 1:NW1
        Error(i,j) = J(j,i) - J_Pres155170.eval([WGin1(i) WGin2(j)]);
    end
end
figure(2)
clf
...
%%
  ...
  options.ub = [1.29, 1.34]';  % Upper bound on the variables.
  ...
  funcs.objective         = @(x)objective (x,J_Pres155170);
  funcs.gradient          = @(x)gradient (x,J_Pres155170);

[xs info] = ipopt(x0,funcs,options);

X_opt = xs
J_opt = J_Pres155170.eval([xs(1) xs(2)]);
GORm1_opt = GORm1_Pres155170.eval([xs(1) xs(2)]);
GORm2_opt = GORm2_Pres155170.eval([xs(1) xs(2)]);
Pwh1_opt = Pwh1_Pres155170.eval([xs(1) xs(2)]);
Pwh2_opt = Pwh2_Pres155170.eval([xs(1) xs(2)]);
Pinl_opt = Pinl_Pres155170.eval([xs(1) xs(2)]);
Zwh1_opt = Zwh1_Pres155170.eval([xs(1) xs(2)]);
Zwh2_opt = Zwh2_Pres155170.eval([xs(1) xs(2)]);
Ztop_opt = Ztop_Pres155170.eval([xs(1) xs(2)]);
WLwh1_opt = WLwh1_Pres155170.eval([xs(1) xs(2)]);
WLwh2_opt = WLwh2_Pres155170.eval([xs(1) xs(2)]);
WGwh1_opt = WGwh1_Pres155170.eval([xs(1) xs(2)]);
WGwh2_opt = WGwh2_Pres155170.eval([xs(1) xs(2)]);
...
disp('*******************************')
J_jac = J_Pres155170.eval_jacobian([xs(1) xs(2)]);
GORm1_jac = GORm1_Pres155170.eval_jacobian([xs(1) xs(2)]);
GORm2_jac = GORm2_Pres155170.eval_jacobian([xs(1) xs(2)]);
Pwh1_jac = Pwh1_Pres155170.eval_jacobian([xs(1) xs(2)]);
Pwh2_jac = Pwh2_Pres155170.eval_jacobian([xs(1) xs(2)]);
Pinl_jac = Pinl_Pres155170.eval_jacobian([xs(1) xs(2)]);
Zwh1_jac = Zwh1_Pres155170.eval_jacobian([xs(1) xs(2)]);
Zwh2_jac = Zwh2_Pres155170.eval_jacobian([xs(1) xs(2)]);
Ztop_jac = Ztop_Pres155170.eval_jacobian([xs(1) xs(2)]);
WLwh1_jac = WLwh1_Pres155170.eval_jacobian([xs(1) xs(2)]);
WLwh2_jac = WLwh2_Pres155170.eval_jacobian([xs(1) xs(2)]);
WGwh1_jac = WGwh1_Pres155170.eval_jacobian([xs(1) xs(2)]);
WGwh2_jac = WGwh2_Pres155170.eval_jacobian([xs(1) xs(2)]);
...
for i=1:200
    for j=1:200
        Z(i,j) = J_Pres155170.eval([X1(i) X2(j)]);
    end
end
%%
figure(3)
