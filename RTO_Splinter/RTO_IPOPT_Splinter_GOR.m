...
%FTH_TOP = BSpline('FTH_TOP_Olga2014.bspline');
...
WL_w160   = BSpline('WL_w160_03_Olga2014_4.bspline');
WG_w160   = BSpline('WG_w160_03_Olga2014_4.bspline');
PWH_w160  = BSpline('PWH_w160_03_Olga2014_4.bspline');
TWH_w160  = BSpline('TWH_w160_03_Olga2014_4.bspline');

WL_w170   = BSpline('WL_w170_03_Olga2014_4.bspline');
WG_w170   = BSpline('WG_w170_03_Olga2014_4.bspline');
PWH_w170  = BSpline('PWH_w170_03_Olga2014_4.bspline');
TWH_w170  = BSpline('TWH_w170_03_Olga2014_4.bspline');

% WL_w155   = BSpline('WL_w155_Olga2014.bspline');
% WG_w155   = BSpline('WG_w155_Olga2014.bspline');
% PWH_w155  = BSpline('PWH_w155_Olga2014.bspline');
% TWH_w155  = BSpline('TWH_w155_Olga2014.bspline');
% 
% WL_w165   = BSpline('WL_w165_Olga2014.bspline');
% WG_w165   = BSpline('WG_w165_Olga2014.bspline');
% PWH_w165  = BSpline('PWH_w165_Olga2014.bspline');
% TWH_w165  = BSpline('TWH_w165_Olga2014.bspline');

w1 = [];
w2 = [];

global P_sep DeltaP1_manifold DeltaP2_manifold espsilon temp_offset k
P_sep = 5.03698;
DeltaP1_manifold = 0.11; %18105;
DeltaP2_manifold = 0.11; %18105;
espsilon = 1e-6;
temp_offset = 0.43;%0.426;
k = 1e5;
...
Pin = 25;
  ...
  options.lb = [0.5, 0.5, 2.0, 2.0, 0.7, 20]';   % Lower bound on the variables.
  options.ub = [1.7, 1.7, 10, 10, 1.1, 28]';  % Upper bound on the variables.
  ...
  options.ipopt.tol                   = 1e-15;
  options.ipopt.constr_viol_tol       = 1e-15;
  options.ipopt.compl_inf_tol         = 1e-15;
  options.ipopt.dual_inf_tol          = 1e-15;
  

  % The callback functions.
  funcs.objective         = @(x)CostFuncScaled_splinter(x,Lambda,w1.FWL,w2.FWL);
  funcs.constraints       = @(x)NonlinConScaled_splinter( x,w1,w2,FP_INL,FT_TOP,FCP_G,FCP_L);
  funcs.gradient          = @(x)gradient_splinter(x,Lambda,w1.FWL,w2.FWL);
  %funcs.gradient          = @(x)Ju_splinter( x,w1,w2,FP_INL,FCP_G,FCP_L,Lambda);
  funcs.jacobian          = @(x)jacobian_splinter( x,w1,w2,FP_INL,FT_TOP,FCP_G,FCP_L);
...
Winj1 = x(1);
Winj2 = x(2);
Pwh1 = x(3) + x(6) + DeltaP1_manifold;
Pwh2 = x(4) + x(6) + DeltaP2_manifold;
Ptop = x(5) + P_sep;
Pin = x(6);
...
WG1 = w1.FWG.eval([Pwh1 Winj1]);
WG2 = w2.FWG.eval([Pwh2 Winj2]);
...
disp(['WG1 = ' num2str(WG1)]);
disp(['WG2 = ' num2str(WG2)]);

disp(['GOR1 = ' num2str(WG1/WL1)]);
disp(['GOR2 = ' num2str(WG2/WL1)]);

[WLtot,ALPHA1,ALPHA2,ALPHA,PT1,PT2,Pin,Ptop,T1,T2,Tin,T_top] = CalculateSetpoints_splinter( xs,w1,w2,FP_INL,FT_TOP,FCP_G,FCP_L);
