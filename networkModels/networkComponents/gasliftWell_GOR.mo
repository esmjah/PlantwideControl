...
Real alpha_G_m_in0(min = -100, max = 100,start=0);
...
//// Liquid Inflow rate
//SI.Pressure P_res(min = 150, max = 200, start=160);
...
alpha_G_m_in0 = 0.01*(par.GOR_nom+GOR)/(1+0.01*(par.GOR_nom+GOR));
Usl_av = (1-(alpha_G_m_in0 + alpha_G_m_in))*par.w_res_in/(par.rho_L*pi*(r_r^2));
...
Usg_av*(rho_G_r*pi*(r_r^2)) = ((alpha_G_m_in0 + alpha_G_m_in)*par.w_res_in+w_G_f);
...
//w_r_in=par.PI*(((((1e5*P_res-P_bh) - 0) ^ 2 + softPApprox) ^ 0.5 / 2 + ((1e5*P_res-P_bh) + 0) / 2));

w_L_r_in = (1-(alpha_G_m_in0 + alpha_G_m_in))*w_r_in;
w_G_r_in = (alpha_G_m_in0 + alpha_G_m_in)*w_r_in;
//// **********  Gas density in bottom of riser **************
